> **Provenance note (Wave 0, RUN_ID 2026-08-31-v1):** This file is a durable, unmodified copy of the
> repo-root legacy file `Claude_Code_Cross-Session_Research_完整建議與經驗_Runbook.md`, checksummed and
> referenced from `research/pitavastatin-ezetimibe-evidence/2026-08-31-v1/00_RUN-MANIFEST.md`.
> Every persistent role and every new session must read this file first (per `CLAUDE.md`) before doing
> any substantive work. The root copy remains untouched as the original legacy input; this copy is the
> one referenced by governance documents so the operating system survives even if the root file is later
> moved or renamed.
